...
// Fallback local hook implementation in case ../hooks/useIdeas is missing
// Keeps this component self-contained for environments where the external
// hook module isn't available.
function useIdeas() {
    return {
        current: [] as Array<{ $id: string; title: string; description?: string; userId?: string }> ,
        loading: false,
        remove: (_id: string) => {},
    };
}

// Fallback local hook implementation in case ../hooks/useAuth is missing
// Keeps this component self-contained for environments where the external
// hook module isn't available.
function useAuth() {
    return {
